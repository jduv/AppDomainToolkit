...
```IAssemblyResolver``` instance, but an overload of ```AppDomainContext.Create()``` allows you to pass your own
...
```IAssemblyResolver``` instance on an ```AppDomainContext``` through the ```context.Resolver``` property.
...
that is the [```RemoteAction```][2] and the [```RemoteFunc```][3]. Please note that both of these classes require _all_
...
credit is due, so the original version of the Remote class lives [here][4]. Alot of the inspiration for this entire
...
[2]: https://github.com/jduv/AppDomainToolkit/blob/master/RemoteAction.cs
[3]: https://github.com/jduv/AppDomainToolkit/blob/master/RemoteFunc.cs
[4]: http://www.superstarcoders.com/blogs/posts/executing-code-in-a-separate-application-domain-using-c-sharp.aspx